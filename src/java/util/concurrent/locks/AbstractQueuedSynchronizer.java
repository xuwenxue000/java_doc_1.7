 ...
 * to synchronization.
 *
 * 提供实现了阻塞锁的框架和关联的同步器,比如那些依赖先进先出的等待队列
 * 这个类是设计用来作为一个有用的基础,为了大部分依赖一个原子int来表示状态这样类型的同步器
 * 子类必须定义这些protected的方法去修改这个状态,并且定义哪一个状态意味着这个对象被获取或释放
 * 考虑这些,其他方法实现所有队列和阻塞的机制
 * 子类可以维护其他状态字段,但只能使用method进行原子性的更新int值(getState,compareAndSetState)
 *
